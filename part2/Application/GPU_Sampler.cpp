...
GLvoid GPU_Sampler::setBitmapData(GLvoid * data, GLuint w, GLuint h, GLuint bpp, GLuint bm)
{
	...
	// if we have a single sampler generate a single ID
	if (samplerType == SINGLE_SAMPLER && bitmaps.empty())
	{
		glGenTextures(1, &bitmap->bitmapID);
	}	

	// then get and set all the data we need
	bitmap->data = data;
	...
	// then calculate the correct external format for the texture
	if (bitmap->bytesPerPixel == 4)
	{
		if (bitmap->bitmapMask == 0x000000ff)
		{
			bitmap->texture_format = GL_RGBA;
		}
		else
		{
			bitmap->texture_format = GL_BGRA;
		}
	}
	else if (bitmap->bytesPerPixel == 3)
	{
		if (bitmap->bitmapMask == 0x000000ff)
		{
			bitmap->texture_format = GL_RGB;
		}
		else
		{
			bitmap->texture_format = GL_BGR;
		}
	}

	// then push back the bitmap into the vector of bitmaps
